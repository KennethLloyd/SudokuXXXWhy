...
int inYGrids(int, int, int, int **, int);
void getYGrids(int**, int);
FILE *f;
int * ygrids;
...
void findSolution(int size, int **puzzle){
  ...
  char * solTypes[] = {"y", "regular", "x", "xy"};  //Four Types of Solution (Mutually Exclusive?)
  ...
  ygrids = (int*) malloc(sizeof(int)*(rowcolTotal+(rowcolTotal/2)));
  ...
  for(h=0; h<3; h++){
    if(solTypes[h] == "y" && rowcolTotal%2==0) {
      break;
    }
...
void getYGrids(int ** numArr, int total){
  int i, j, count=0;
  for(i=0; i<total; i++){
    for(j=0; j<total; j++){
      if(i==j && i < (total/2) || ((i+j)==(total-1)) && i < (total/2) || j == (total/2) && i >= (total/2)){
        ygrids[count++] = numArr[i][j]; 
      }
    }
  }
}

int inYGrids(int x, int row, int col, int ** numArr, int total){
  int i,j;
...
int checkPossible(int *candidates, char * solutionType, int size, int row, int col, int ** grid){
  int i, j, k, l, m, flag, counter=0, subgridSize, *numcounter, *numcountery;
  subgridSize = sqrt(size);
  numcounter = (int*)malloc(sizeof(int)*(size+2));
  numcountery = (int*)malloc(sizeof(int)*(size+2));
  for(i=0; i<size+2; i++){
    numcounter[i] = 0;
    numcountery[i] = 0;
  }
  ...
  if(grid[row][col] != 0){
    *(candidates+counter) = grid[row][col];
    ...
    for(i=size; i>0; i--){    //PRESORTS IN DESCENDING ORDER
      ...
      for(j=0; j<size; j++){
        if(solutionType == "regular" || solutionType == "x" || solutionType == "y"){
